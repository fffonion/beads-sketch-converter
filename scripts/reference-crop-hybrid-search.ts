...
const OUTPUT_DIR = join(process.cwd(), "output", "reference-crop-hybrid-search");
const GRID_SIZE = 40;
...
async function main() {
  ...
  const baselineCrop = loadRefinedCrop();

  const refined = searchBestCrop(source, targetCells, {
    sizeFrom: Math.max(880, baselineCrop.size - 80),
    sizeTo: Math.min(source.width, baselineCrop.size + 64),
    sizeStep: 8,
    leftStep: 4,
    topStep: 4,
    leftFrom: Math.max(0, baselineCrop.left - 32),
    leftTo: Math.min(source.width - 1, baselineCrop.left + 48),
    topFrom: Math.max(0, baselineCrop.top - 40),
    topTo: Math.min(source.height - 1, baselineCrop.top + 40),
  });
  ...
  writeBmp(join(OUTPUT_DIR, "best-crop-window.bmp"), renderCropPreview(source, refined, 900));
  writeFileSync(
  ...
  console.log(JSON.stringify({ baselineCrop, refined }, null, 2));
}
...
function searchBestCrop(
  ...
  for (let size = options.sizeFrom; size <= options.sizeTo; size += options.sizeStep) {
    ...
    const leftTo = Math.min(options.leftTo, source.width - size);
    const topTo = Math.min(options.topTo, source.height - size);
    for (let left = options.leftFrom; left <= leftTo; left += options.leftStep) {
      for (let top = options.topFrom; top <= topTo; top += options.topStep) {
...
function sampleCropGrid(source: RasterImage, crop: CropBox, width: number, height: number) {
  ...
  for (let row = 0; row < height; row += 1) {
    ...
    for (let column = 0; column < width; column += 1) {
      ...
      cells.push(sampleHybridPatch(source, Math.round(left), Math.round(top), Math.round(right), Math.round(bottom)));
    }
  }
  return cells;
}
