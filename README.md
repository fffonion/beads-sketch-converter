...
一个基于 Bun、React、TypeScript、Tailwind CSS 和 Radix UI 的前端工具，用来把图片转换成拼豆图纸，并在网页里继续编辑、查看、计时和导出。
...
- 支持拼豆模式高亮单色、全屏查看、计时器、翻转、显示颜色名称
- 支持保存带元数据的 PNG，重新导入时可直接读取，不必重新判格
...
1. 打开页面后，在 landing page 导入图片或现有图纸。
2. 导入完成后进入完整工作区。
3. 左侧用于原图、裁切、网格策略与图像整理。
4. 右侧用于像素编辑或拼豆模式查看。
5. 最后保存生成的图纸 PNG。
...
- 保存文件名默认为 `【拼豆豆】原始文件名.png`
- 导出的 PNG 带品牌标题、logo、色系信息、总颗粒数、颜色统计
- 导出的 PNG 写入 `pindou-chart` 元数据
- 再次导入我们自己保存的图纸时，会优先直接读取元数据，不再重新 parse 图片
